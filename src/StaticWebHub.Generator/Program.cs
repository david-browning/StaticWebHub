...
public static class Program
{
      ...
      CancellationToken cancellationToken)
   {
      ...
      var services = SetupServices(contentStore, renderer, resolver);
      ...
      var validators = services.GetServices<ITypedPageValidator>();
      IPageValidator pageValidator = new PolymorphicPageValidator(validators);
      IPageRenderer pageRenderer = new BasicPageRenderer();
      IPageWriter pageWriter = new FileSystemPageWriter(outputRoot);
      var generator = new SiteGenerator(
         pageSource, pageDeserializer, pageValidator, pageRenderer, pageWriter);
   ...
   private static ServiceProvider SetupServices(
      ...
      IAssetRenderer assetRenderer,
      IPageAssetResolver resolver)
   {
      var services = new ServiceCollection();
      services.AddSingleton<IContentStore>(contentStore);
      services.AddSingleton<IAssetRenderer>(assetRenderer);
      services.AddSingleton<IPageAssetResolver>(resolver);
      services.AddPageValidators();
   ...
   private static void PrintValidationMessage(ValidationMessage message)
   {
   ...
   {
      if (string.IsNullOrEmpty(message.PageId))
      {
         return $"{message.Severity} {message.Code}: {message.Message}";
      }
      else
      {
         return $"{message.Severity} {message.Code}: Page Id: {message.PageId}: {message.Message}";
      }
   }
